...
# Fit GARCH(1,1) model to diff(log_price)
garch_model <- garchFit(~ garch(1,1), data = train_series, cond.dist = "std", trace = FALSE)
summary(garch_model)

# Forecast next n steps ahead
n_forecast <- nrow(test_data)
garch_forecast <- predict(garch_model, n.ahead = n_forecast, mse = "cond")
predicted_dlog <- garch_forecast$meanForecast

# Reconstruct log price and back-transform
last_log <- tail(train_data$log_price, 1)
reconstruct_log <- function(last_log, diffs) {
  cumsum(c(last_log, diffs))[-1]
}
predicted_log <- reconstruct_log(last_log, predicted_dlog)
...
MAPE <- mean(abs(residuals / actual_price)) * 100
...
ggplot(garch_forecast_df, aes(x = Date)) +
  ...
  labs(title = "GARCH Forecast vs Actual Cocoa Prices",
       subtitle = "Dashed = GARCH Forecast, Solid = Actual",
       y = "Price (USD/tonne)", x = "Date") +
  theme_minimal()
```
# New GARCH + ARIMAX(0,0,1)
```{r}
# Load packages
library(rugarch)
library(ggplot2)
...
# 3. Forecast step-by-step
n_forecast <- nrow(test_data)
forecasted_log <- numeric(n_forecast)
...
for (i in 1:n_forecast) {
  ...
  forecasted_log[i] <- fitted(fc)[1]
}

# 4. Reconstruct predicted prices
predicted_price <- exp(forecasted_log)
actual_price <- test_data$Price
...
ggplot(forecast_df, aes(x = Date)) +
  geom_line(aes(y = Actual), color = "black", linewidth = 1) +
  geom_line(aes(y = Forecast), color = "blue", linetype = "dashed", linewidth = 1) +
  labs(title = "Figure 6: ARIMAX(0,0,1) + GARCH(1,1) Forecast vs Actual Cocoa Prices",
...
resid_garch <- residuals(fit, standardize = TRUE)
plot(resid_garch)
# ACF of residuals
acf(resid_garch, main = "Figure 7: ACF plot of standardized residuals for GARCH model")

# Q-Q plot
qqnorm(resid_garch, main = "Figure 8: Q-Q plot of standardized residuals from GARCH model")
qqline(resid_garch)

# Ljung-Box test for independence
Box.test(resid_garch, lag = 20, type = "Ljung-Box")

# ARCH LM test for remaining ARCH effects
ArchTest(resid_garch, lags = 12)
```
